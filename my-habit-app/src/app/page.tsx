                      ...
                      <React.Fragment key={`${routine.task}-${idx}`}>
                        <div className="border rounded p-4 flex justify-between items-center">
                          ...
                          <input
                            ...
                            onChange={() => toggleDone(routines.indexOf(routine))}
                          />
                        </div>
                        {routine.done && (
                      ...
                      data={[
                        {
                          ...
                          Completion: Math.round(
                            (routines.filter((r) => r.day === selectedDay && r.done).length /
                              (routines.filter((r) => r.day === selectedDay).length || 1)) *
                              100
                          ),
                          Satisfaction: (() => {
                            const doneArr = routines.filter(
                              (r) => r.day === selectedDay && r.done
                            );
                            return doneArr.length
                              ? Math.round(
                                  doneArr.reduce((s, r) => s + r.rating, 0) /
                                    doneArr.length
                                )
                              : 0;
                          })(),
                    ...
                    <BarChart
                      data={(() => {
                        ...
                        for (let d = 1; d <= daysInMonth; d++) {
                          ...
                          if (
                            dt.getDay() === fullDays.indexOf(selectedDay) + 1
                          ) {
                            const iso = dt
                              .toISOString()
                              .split("T")[0];
                            ...
                            const doneCount = dayRoutines.filter(
                              (r) => r.done
                            ).length;
                            const sat = doneCount
                              ? Math.round(
